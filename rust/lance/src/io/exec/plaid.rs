...
use crate::index::plaid::{PlaidVectorIndex, is_plaid_index_metadata, query_to_array};
...
const CANDIDATE_TIME: &str = "plaid_candidate_time";
...
async fn execute_search(
    ...
    let refine = query
        ...
        .max(1);
    let candidate_limit = query.k.saturating_mul(refine).max(query.k);
    ...
    for metadata in &indices {
        ...
        let params = plaid.candidate_params(&query, candidate_limit);
        ...
        let (hits, stats) = spawn_cpu(move || {
            ...
            index.search_candidates(query_for_cpu.view(), &params, mask_for_cpu.as_ref())
        })
        .await?;
    ...
    let row_addresses = candidates
        ...
        .collect::<Vec<_>>();
    let projection = Arc::new(dataset.schema().project(&[query.column.as_str()])?);
    let raw_fetch_started = Instant::now();
    let batch = dataset
        .take_rows(&row_addresses, ProjectionRequest::Schema(projection))
        .await?;
...
fn exact_scores(
    ...
    for row_index in 0..documents.len() {
        ...
        hits.push(ExactHit {
            row_address: 0,
            distance: query.nrows() as f32 - score,
        });
    }
    Ok(hits)
}
